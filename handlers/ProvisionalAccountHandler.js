...
exports.findProvisionalAccount = function(req, res , next) {
    ...
    provisionalAccounts.find({provisionalAccountID : req.params.provisionalAccountID} , function(provAcctError , provAcctSuccess) {
		...
		// set accessToken from the header
	        // Note: it does NOT work for login since the access token does NOT include the right scope.....need to check it with RSE team
    		console.log("Authorization token: " + req.header('Authorization'));
		provisionalAccount.accessToken = req.header('Authorization').substring(7);
    		console.log("provisionalAccount.accessToken: " + provisionalAccount.accessToken);
